...
class ControllerList {
    ...
    function __construct() {
        ...
        // Get all controllers
        $this->setControllers();
        $this->writeControllers($this->aControllers);
    }
    ...
    /**
     * Escreve os métodos no banco de dados
     * @param  [type] $array [description]
     * @return [type] [description]
     */
    private function writeControllers($array){
    	/* class */
    	foreach ($array as $class => $methods){
    		/* methods of class */
    		foreach ($methods as $method){
    			$full = "$class/$method";
    			if($this->getControllerFull($full) == 0){
					$Db = [
						'CLASS' => strtolower($class),
						'METHOD' => strtolower($method),
						'CLASSMETHOD' => strtolower($full),
						'DESCRIPTION' => 'Aguardando descrição',
						'DTCAD' => date("Y-m-d H:i:s"),
						'VISIBILITY' => 1,
						'STATUS' => 1,
		    		]; $this->CI->db->insert('rotas', $Db);
    			}
    		}
    	} return;
    }
    ...
    /**
     * Verifica se esta rota já existe no banco de dados
     * @param  [type] $full [description]
     * @return [type] [description]
     */
    public function getControllerFull($full){
    	/* processamento e verificação */
    	$this->CI->db->where('CLASSMETHOD', $full);
    	return $this->CI->db->get('rotas')->num_rows();
    }
}
// EOF